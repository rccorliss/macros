...
void TPC_Cells()
{
  ...
  edrift->Detector("TPC");
  edrift->set_double_param("max_time", 13200);  // ns 
  edrift->set_double_param("extended_readout_time", 7000);  // ns - for pp mode, 7 microseconds for extended readout
  //edrift->set_double_param("extended_readout_time", 0.0);  // ns - for pp mode, 7 microseconds for extended readout
...
void TPC_Clustering()
{
  ...
  if( G4TPC::ENABLE_CENTRAL_MEMBRANE_HITS )
    {
      // combine CM clusters in adjacent padrows and put on node tree
      auto cm_clusterizer = new PHTpcCentralMembraneClusterizer;
      cm_clusterizer->Verbosity(verbosity);
      cm_clusterizer->set_histos_on(true);
      se->registerSubsystem(cm_clusterizer);

      // Match CM clusters to stripes and calculate differences
      auto cm_matcher = new PHTpcCentralMembraneMatcher;
      cm_matcher->Verbosity(verbosity);
      cm_matcher->set_histos_on(true);
      se->registerSubsystem(cm_matcher);
    }
